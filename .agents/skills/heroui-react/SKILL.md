...
Check UI_STYLE_GUIDE.md as reference for styling, colors and general guidelines. It is generated based on the Flutter app, so it may not be entirely accurate for React. For best results, refer to official HeroUI documentation but use the style guide as a reference for what colors, border radiuses, paddings, etc. to use.
```
